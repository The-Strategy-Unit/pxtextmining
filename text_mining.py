...
# Code adopted from:
# https://scikit-learn.org/stable/auto_examples/text/plot_document_classification_20newsgroups.html?highlight=text%20classification%20sparse
...
data_train, data_test = train_test_split(
    text_data, test_size=0.33, random_state=42, stratify=text_data.target)
...
if opts.use_hashing:
    ...
    X_train = vectorizer.transform(data_train['improve'].values.tolist())
else:
    vectorizer = TfidfVectorizer(sublinear_tf=True, max_df=0.5,
                                 stop_words='english')
    X_train = vectorizer.fit_transform(data_train['improve'].values.tolist())
duration = time() - t0
...
t0 = time()
X_test = vectorizer.transform(data_test['improve'].values.tolist())
duration = time() - t0
